...
class AdminViewController < ApplicationController
  ...
  def index
    ...
    @verified = AdminVerified.search(params[:search]).order("admin_verifieds.created_at ASC").page(params[:page]).per(10)
  ...
  def create
    ...
    respond_to do |format|
      if @event.save
        ...
        format.html { redirect_to :controller => :admin_view, :action => :index }
      else
